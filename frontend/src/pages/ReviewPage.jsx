...
export default function Review() {
  ...
  const restaurant = useMemo(
    () => ({
      ...
      hours: [
        ...
        { day: "Tuesday", time: "7am - 10pm", open: true },
    ...
    () => [
      {
        ...
        rating: 4,
        text:
          "Lovely customer service! Had my bowl ready quickly and was so good! Going to make it a habit to continue to go to shakesmart!",
      ...
      },
      {
        ...
        rating: 5,
        text:
          "Great people who work here, always happy to see everyone. Excellent food and shakes. Love their vegan options.",
        tags: ["vegan", "quality"],
        images: [rev3],
      },
      {
  ...
  const ratingCounts = useMemo(() => ({ 5: 26, 4: 15, 3: 8, 2: 3, 1: 2 }), []);
  ...
  <button
    ...
    aria-label="Go home"
  >
    <img className="review__logoImg" src={logo} alt="umami logo" />
  </button>

  <button className="review__profile" aria-label="Profile">
    <span className="review__profileIcon">👤</span>
  </button>
</header>
      ...
      >
        ...
        <div className="review__heroContent">
          <h1 className="review__title">{restaurant.name}</h1>
...
>
  <span className="review__photosDot" /> view photos
</button>

        </div>
      </section>

      <nav className="review__tabs" aria-label="Sections">
        <button
      ...
      <main className="review__main">
        <section className="card card--section" id="section-menu">
  ...
  {restaurant.menuImages.map((src, idx) => (
    <div key={idx} className="review__menuImgWrap">
      <img
  ...
  <button className="review__carouselNext" aria-label="Next">
    ›
  </button>
</div>


          <button className="pillBtn pillBtn--ghost">👁 view menu and nutrition</button>
        </section>

        <section className="card card--section" id="section-info">
          <h2 className="review__h2">Info</h2>

          <div className="review__infoGrid">
            <div className="review__hours">
              <div className="review__subHeader">
                <span className="review__subIcon">🕒</span>
                <span className="review__subTitle">Hours</span>
              </div>

              <div className="review__hoursList">
                {restaurant.hours.map((h) => (
                  <div key={h.day} className="review__hoursRow">
                    <span className="review__day">{h.day}</span>
                    <span className="review__time">{h.time}</span>
                    {h.open && <span className="review__open">open</span>}
                  </div>
                ))}
              </div>

              <button className="pillBtn pillBtn--orange">order in-person</button>
            </div>
            ...
            <div className="review__mapBlock">
              ...
              <div className="review__locationChipRow">
                <span className="chip chip--outline">{restaurant.locationLabel}</span>
              </div>
            </div>
          </div>
          ...
          <div className="review__peak">
            <div className="review__subHeader">
              <span className="review__subIcon">⌛</span>
              <span className="review__subTitle">Peak Hours</span>
            </div>
  ...
  {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
    ...
    >
      {d}
        ...
        <section className="review__reviewsGrid" id="section-reviews">
          <div className="card card--section">
            <div className="review__reviewsHeader">
              <h2 className="review__h2">Reviews</h2>

              <div className="review__reviewsControls">
                <button className="pillBtn">✎ write review</button>
                <button className="pillBtn pillBtn--ghost">filter ▼</button>
              </div>
            </div>
            ...
            <div className="review__pagination">‹ 1 2 3 ›</div>
          </div>

          <aside className="card card--section review__ratingCard">
            <div className="review__ratingTop">
              <div className="review__ratingValue">{restaurant.rating.toFixed(1)}</div>
              <div className="review__ratingLabel">
                Overall Rating <span className="review__starSmall">★</span>
              </div>
              <div className="review__ratingCount">{restaurant.ratingCount} reviews</div>
            </div>

            <div className="review__bars">
              {[5, 4, 3, 2, 1].map((s) => (
                <RatingBar key={s} star={s} count={ratingCounts[s]} max={ratingCounts[5]} />
              ))}
            </div>
          </aside>
        </section>
  ...
  return (
    <div className="stars" aria-label={`Rating ${value}`}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        return (
          <span key={i} className={`star ${isFull ? "is-full" : ""} ${isHalf ? "is-half" : ""}`}>
            ★
          </span>
        );
      })}
  ...
  return (
    <article className="reviewCard">
      <div className="reviewCard__top">
        <div className="reviewCard__left">
          ...
          <div>
            <div className="reviewCard__nameRow">
              ...
              {review.verified && <span className="badge badge--verified">Verified Cal Poly Account</span>}
            </div>
        ...
        <button className="reviewCard__helpful" aria-label="Helpful">
          👍 <span>helpful</span>
        </button>
      </div>
      ...
      {review.images.length > 0 && (
        <div className="reviewCard__imgs">
          {review.images.map((src, idx) => (
            <img key={idx} src={src} alt={`review photo ${idx + 1}`} />
          ))}
        </div>
      )}
    </article>
  ...
  return (
    <div className="ratingRow">
      ...
      <div className="ratingRow__track">
        <div className="ratingRow__fill" style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
